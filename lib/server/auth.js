...
export async function validateArtisanUserAccountInput(input) {
    const name = String(input.name || "").trim();
    const username = normalizeUsername(input.username);
    const email = String(input.email || "").trim().toLowerCase();
    const password = String(input.password || "");
    ...
    if (!name || !artisanId || !isValidUsername(username) || password.length < 6) {
        return { error: "Tên đăng nhập hợp lệ và mật khẩu từ 6 ký tự là bắt buộc." };
    }
    ...
    return { data: { name, username, email, password, artisanId } };
}

export async function createArtisanUserAccount(input) {
    const parsed = await validateArtisanUserAccountInput(input);
    if (parsed.error) return parsed;
    const { name, username, email, password, artisanId } = parsed.data;
    const user = {
        id: `user_${Date.now()}`,
        username,
        email: email || buildLocalEmail(username),
